...
This is K3NG's modified squeezer paddle for Arduino Uno, Arduino Nano or Nano clones.
For the first use you have to erase the EEPROM memory by pressing both paddles and rebooting (either by pressing the reset button or power-on reset) the microcontroller.
Othewise speed changings will have no effect.
Hardware modifications: Install a second 10k pot with the wiper connected to analog input A1 (in this version there are no memory button functions provided).  
...
//DF4IAN
void check_sidetone_potentiometer()
{
  int raw = analogRead(A1);
  int freq = map(raw, 0, 1023, 
                 sidetone_hz_limit_low + 1,
                 sidetone_hz_limit_high - 1);
  
  if (abs(freq - configuration.hz_sidetone) > 10) {
    configuration.hz_sidetone = freq;
  }
}
